<div class="mt-5 pt-4 d-flex align-items-center">
    <a href="<?= base_url('Camp/center') ?>" class="btn ms-3">
...
<div class="mt-4 bg-white p-4 text-dark">
    ...
    <form action="<?= base_url('Camp/update_center') ?>" method="POST" class="row g-3 needs-validation" novalidate>
...
<script>
    function validasi_modem(element, id, id_message) {
        ...
        if (!$(element).val().match(number)) {
            ...
            // AJAX
            $.ajax({
                url: '<?= base_url('Camp/select_not_senter_js') ?>',
                method: 'POST',
                data: {
                    id: id,
                },
